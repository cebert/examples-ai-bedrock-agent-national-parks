 ...
 * Provides methods to fetch park data and handle errors.
 */
export class NPSClient {
   ...
   /**
    * Creates an instance of the NPSClient.
    *
    * @param config - Configuration options for the client.
    */
   constructor(config: NPSClientConfig) {
      if (!config.apiKey) {
         throw new NPSAuthenticationError(
            'API key is required to initialize the NPS client',
         );
      }
      ...
      this.defaultLimit =
         typeof config.defaultLimit === 'number' ? config.defaultLimit : 50;
      this.timeout =
         typeof config.timeout === 'number' ? config.timeout : 30000;
   }

   /**
    * Fetches a list of parks based on provided filters.
    *
   ...
   /**
    * Fetches detailed information about a specific park by its code.
    *
   ...
   async getPark(parkCode: string): Promise<Park> {
      ...
      const response = await this.getParks({
    ...
    */
   private getParkUrl(parkCode: string): string {
